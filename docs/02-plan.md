...
Resolved into the current design: Bun/native production integration, user/machine token claims, exact-pair machine ceilings, issuing the entitled scope subset at authorisation, no-CDN posture and encrypted Google/upstream token storage. Secret delivery and recovery remain operational inputs. Own-tenant SSO, verified linking, five-minute sensitive-command freshness and terminal `deletedAt` are accepted decisions. UUID audit history needs no named-recovery feature; physical product purge and its duration are deferred.

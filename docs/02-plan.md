...
| 1 | **Schema foundation** | Bun + Hono + Better Auth 1.7.2 + Drizzle skeleton; UUIDv7 schema in [`04-answerable-id-schema.md`](04-answerable-id-schema.md); `/healthz`, `/readyz`, admin OpenAPI infrastructure, the public contract at `/openapi.json` (allowlist-derived, snapshotted in `apps/id/openapi.json` for the docs site), disposable-database schema push, full-coverage tests, and a deny-by-default Better Auth HTTP allowlist. Production migration and admin write routes follow schema approval |

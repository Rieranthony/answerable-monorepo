...
Last updated 2026-09-08. This is the only file that carries dates.

**Foundation gate before expansion.** The proposed [enterprise foundation specification](05-id-enterprise-foundation.md) addresses the [code review findings](../reports/answerable-id-feedback-review.md), tenant isolation, repeat-safe administration and durable audit evidence. Its F0–F7 slices are **Not yet implemented**. Complete this gate before production machine administration, public tenant configuration writes or opening user OAuth grants. It strengthens the existing build order; it does not replace the real-tenant validation, lifecycle, key-custody or infrastructure gates below.
...
| 1 | **Schema foundation** | Bun + Hono + Better Auth 1.7.2 + Drizzle skeleton; UUIDv7 schema in [`04-answerable-id-schema.md`](04-answerable-id-schema.md); `/healthz`, `/readyz`, admin OpenAPI infrastructure, the public contract at `/openapi.json` (allowlist-derived, snapshotted in `apps/id/openapi.json` for the docs site), disposable-database schema push, full-coverage tests, and a deny-by-default Better Auth HTTP allowlist. Production migration and admin write routes are steps 2a and 2b |
| 2 | **Organizations + upstream federation** | SSO plugin and per-org tenant-scoped issuer rows; against the in-process IdP stub: own org accepted with an active verified user, immutable directory columns, and one `member` row; foreign issuer and Entra `tid` rejected; both Entra guest signals rejected; personal Google, hosted-domain mismatch, unverified Google/generic email, wrong domain, and cross-org domain ambiguity rejected; inert `(tid, oid)` placeholder import binds exactly once; disabled recycled addresses are retired while active/inert collisions and Better Auth email-linking are rejected; disabled bound users rejected; runtime discovery works; resolver exceptions roll back and redirect with `SSO_USER_RESOLUTION_FAILED`; the public SSO/session route inventory remains deny-by-default |
| 2a | **First production migration** | `apps/id/drizzle` generated from the approved schema; the test database is built from the migrations so the catalog contract test guards them; a drift test fails when schema and migrations disagree |
| 2b | **Admin API, platform tier** | One API at `/api/admin/v1`, two tiers, every route tagged `x-tier`; authority is an effective membership plus an entitlement to the admin API resource with one of six scopes, no roles; humans use the session cookie, machines use `client_credentials` at `POST /auth/oauth2/token` (the only open grant); `audit_events` records every write, denial, sign-in outcome and sign-out; the platform organisation, admin resource and staff group are seeded at startup and a self-locking root secret (`ROOT_ADMIN_SECRET`, break-glass with `ROOT_ADMIN_BREAK_GLASS`) lets the first staff member be created with the API alone, so there are no seed accounts and no CLIs; families: organisations, domains, SSO provider, users, sessions, members, groups, clients, resources, entitlements, access views, audit, platform and organisation summaries, sign-in diagnosis, SSO connectivity test, cross-organisation entitlement view, per-person audit trail; every operation carries an agent-facing description, `x-kind` and `x-scopes` so it can be wrapped as an MCP tool, and erasure confirms through a `confirm` query parameter; every route has the generated negative suite, hand-written happy paths per tier, and passes the union guard against the OpenAPI document; the admin contract is published in the docs |
| 2b.1 | **Enterprise foundation — proposed, Not yet** | [F0–F7](05-id-enterprise-foundation.md#8-implementation-sequence-and-proof): immutable client/system identity and strict token binding; durable audit subjects and token outcomes; transactional operation journal and replay/revision contract; tenant-local membership/session lifecycle; platform capability ceilings and paired assignments; isolation, concurrency, fault and migration tests; documentation updated with each implementation slice |
| 2c | **Org self-service, tenant tier** | The `org:*` routes usable by a client organisation's admin over the public internet; DNS domain verification (`verified_at`); SSO self-setup with an admin-consent link and a test-connection check; the catalog-versus-assignment decision so organisation admins can narrow app access; `allow_guest_accounts`; tenant rate limits; tenant docs |
| 3 | **OIDC provider for apps** | First commit allowlists `/auth/oauth2/authorize`, `/auth/oauth2/consent`, and `/auth/oauth2/continue`; first-party clients use `private_key_jwt`; the `organization` authorize parameter routes a login to the org's IdP; "Use another email" is the escape and staff-routing rule, overriding that route by email domain; auto-redirect; discovery + JWKS conformance tests; a stub "cell" logs in end to end; discovery and JWKS are served at the root because the issuer is the bare origin |
| 3a | **Console** | A thin signed-in area in `apps/web` for both tiers on top of the admin API |
| 4 | **Entitlements** | Extend the implemented schema in [`04-answerable-id-schema.md`](04-answerable-id-schema.md) with the proposed [tenant ceilings and exact client–resource pairs](05-id-enterprise-foundation.md#one-policy-explicit-intersections); the single policy runs at authorize, refresh, and client-credentials; the effective-window predicate is part of the policy, with boundary tests for expired and future rows; negative tests for cross-org access, audience swapping, unentitled clients, registration-granted scopes and tenant self-escalation |
...
| 6 | **Identity linking + migration** | Bulk import as inert records; binding on `(tid, oid)` / `(issuer, sub)` at first login; the import sources `oid` from Graph, never the cells' `openidId` (a pairwise `sub`); duplicates/changed emails fail closed; `legacyOpenidId` preserved; rollback = environment change, tested |
| 7 | **Lifecycle** | Bounded sessions; upstream re-check as a **top-level redirect** (not an iframe — `Q-AID-RECHECK`); refresh rotation with reuse detection (family kill); back-channel logout emission; admin kill switch (`ocadmin deactivate-users`); background delegation that outlives the browser session and re-checks at every renewal; audit log export and the "token after deprovision" alert; session deletion through Better Auth's endpoint context (`runWithEndpointContext`) so back-channel logout fires; the offboarding targets (no new tokens ≤ 5 min, all access expired ≤ 15–30 min) as tests |
...
| `Q-GOOGLE-SECRET-CUSTODY` | Google requires a client secret stored in plaintext inside `oidc_config`; Entra uses `private_key_jwt` plus `resolvePrivateKey`. Decide Google secret custody in the key milestone | Step 8 | Decide in the key milestone |

...
**MCP foundation:** [Shared creation and end-to-end validation](07-mcp-platform-draft.md) is implemented and locally accepted: `bun run mcp:test:e2e` signs organisations into the e2e MCP through real ID with the official MCP OAuth client. The [evidence](../reports/mcp-foundation-evidence.md) records the tested scope. Open: partial entitlements ([Q-SCOPE-SUBSET](#open-register)) and automatic client registration ([Q-MCP-CLIENT-REGISTRATION](#open-register)).
...
| `Q-SCOPE-SUBSET` | An organisation entitled to only some of an MCP's scopes is refused, because `grant-scopes.ts` refuses a request above any ceiling and ID fills an omitted scope with the client's registered scopes. Standard MCP clients always ask for every advertised scope. Issue the entitled subset at authorisation time instead; refresh still never widens | Read-only or group-scoped MCP access |
| `Q-MCP-CLIENT-REGISTRATION` | Hosts need a pre-registered client today. Claude Code and Claude.ai prefer client ID metadata documents (`@better-auth/cimd`, which needs Better Auth 1.7.6) and fall back to dynamic registration. Decide which to support and how an organisation approves such a client | Claude.ai connectors without manual client setup |

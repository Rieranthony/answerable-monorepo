...
Removing the last platform administrator through membership, group or capability changes is allowed. Recover with the root secret and `ROOT_ADMIN_BREAK_GLASS=true`. Global user disable and erasure retain their last-writer safeguard.
...
Public identifiers remain reserved after deletion because the tombstone row keeps the unique identifier; owner/configuration changes cannot adopt old credentials. Tokens bind immutable subject, tenant/client/resource instances and authorisation versions. User grant authentication provenance is immutable. Missing provenance cannot be inferred from a later session.
...
Recovery must preserve retained keys, reservations, receipts and revocation barriers. A snapshot can predate acknowledged changes. Not yet. A restore drill against production-shaped data is a release input.

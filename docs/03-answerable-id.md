...
Machine-to-machine callers acting purely as themselves (the provisioning dispatcher, backup jobs, a service's own health checks) use client-credentials grants, which authenticate the _service itself_, not a user. They never receive user-subject tokens, and they never accept a user ID as a substitute for one. A service token plus "now act as Alice" is exactly the shortcut this design rejects; when a service acts for a person, it uses one of the paths above. Where a service-as-itself action is triggered by a person, such as an admin running a bulk operation, the requesting user is recorded in the audit trail as data, without that data conferring any authority.
...
- **KMS-backed signing** where the key can be _used_ but never _extracted_, if Better Auth's JWT plugin supports it reliably. Otherwise, a key-encryption key held **outside** Postgres and outside the app's general secret store, with the residual combined-compromise risk documented explicitly.
...
**Addendum (build).** The admin API lives at `/api/admin/v1` in two tiers. Platform-tier routes (Answerable staff managing every organisation) stay tailnet-only as above. Tenant-tier routes let an organisation manage itself and are public, protected by the organisation's own SSO, an entitlement to the admin API resource carrying `org:*` scopes, and rate limits. Every route carries its tier in the OpenAPI document (`x-tier`); the ingress enforces the split. Authority is never a role: it is an effective membership plus an entitlement, re-read on every request, and every write and denial is in the audit log. There are no seed accounts: the platform organisation, the admin API resource and the staff group are seeded at startup, and a root secret held outside the database (`ROOT_ADMIN_SECRET`) authenticates a break-glass principal that satisfies every platform scope, is audited on every request, and refuses itself once a person holds `platform:write` unless break-glass is explicitly re-enabled at deploy time.
...
**Targeted tenant RLS.** The initial migration protects groups, group_members and entitlements using transaction-local tenant/platform read/write scopes. Missing scope denies access; broker grants are limited to the authenticated user's membership organisations, and root-lock checks to the immutable platform organisation. Bootstrap uses its established tenant UUID. Broker savepoints restore enclosing scope. Runtime startup rejects disabled RLS. See [the isolation inventory](../reports/answerable-id-isolation-inventory.md) for exclusions and rollout limits. Query context types, access ceilings and production migration rehearsal remain unfinished.

**Current pair implementation:** entitlement configuration and access projections preserve exact client/resource pairs. Pair scopes are isolated from client-only and resource-only targets and cannot satisfy direct administrative grants or platform-writer detection. The common token evaluator and capability ceiling described above remain unfinished; user OAuth is still closed. Legacy non-admin resource-only assignments require approved conversion before that gate opens.

**Current machine capability enforcement:** platform-managed exact owner/client/resource ceilings now constrain machine issuance and online admin authority. Scope defaults and explicit requests obey client/resource/capability intersection. Capability changes/removal use the existing command journal, current platform contexts, revisions and before/after audit; tenant RLS cannot write ceilings. The organisation lock orders capability writes against issuance. User/direct-session ceilings, the complete shared evaluator and trusted user grant context remain unfinished. See [implementation contract](../reports/answerable-id-foundation-execution-plan.md).

**Direct administrator ceiling is implemented.** Effective resource-only assignments now intersect an organisation's `admin_session` capability and the enabled admin resource vocabulary. Bootstrap seeds the reserved platform ceiling under immutable system bindings and preserves its restrictions on restart. The initial migration prohibits deletion of that reserved row. Capability commands support direct sessions and machines, with current authority, journalled recovery and revisions. Tenant ceilings require explicit platform approval. Capability changes protect the current last platform writer; complete user/group/member writer protection and tenant-bound user OAuth remain **Not yet**. See the [foundation plan](05-id-enterprise-foundation.md) and [execution evidence](../reports/answerable-id-foundation-execution-plan.md#direct-administrator-capability-enforcement).

**Global erasure effect contract.** Global user erasure now records version-two local cascade evidence, including memberships and policy sources across tenants, owned-client effects on other users, deleted credential-row identities, cleared surviving token/session references and SSO attribution revisions. Actual mutation rows supply the manifest in the operation transaction; credential values and account linkage identifiers are excluded. External client references still prevent erasure. This does not turn an internal account UUID into recoverable named identity, compute complete effective access or prove downstream logout. See the [implemented schema contract](04-answerable-id-schema.md) and [execution evidence](../reports/answerable-id-foundation-execution-plan.md#global-user-erasure-cascade-evidence).

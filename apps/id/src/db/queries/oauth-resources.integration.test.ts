...
import * as queries from "./oauth-resources.ts";
...
beforeEach(async () => {
  await connection.db.execute(
    sql`truncate table organizations, oauth_resources cascade`,
  );
});
afterAll(async () => {
...
test("resource queries cover CRUD, filters, cursors, locks and entitlement references", async () => {
  ...
  expect(
    await db.transaction((tx) => queries.lockResource(tx, a!.identifier)),
  ).toEqual(a!);
  expect(

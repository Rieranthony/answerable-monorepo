...
import * as queries from "./organization-domains.ts";
test("domain queries scope rows, paginate newest first, filter and enforce active ownership", async () => {
  ...
  expect(
    await queries.findOrganizationByDomain(db, "FIRST.EXAMPLE.COM"),
  ).toEqual({ id: a.id, slug: a.slug });
  await expect(
  ...
  expect(await queries.findOrganizationByDomain(db, first.domain)).toBeNull();
  expect(
    (
      await queries.listOrganizationDomains(db, a.id, {
        limit: 10,

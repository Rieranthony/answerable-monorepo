...
`src/env.ts` validates `Bun.env` once during startup and returns a typed configuration object. `DATABASE_URL` and `BETTER_AUTH_SECRET` are required and produce a named startup error when absent. `BETTER_AUTH_URL` defaults to `https://id.answerable.org` when `NODE_ENV=production` and the value is unset or blank; explicit values override it. Development and test require an explicit URL, normally `http://localhost:47300`. `BETTER_AUTH_TRUSTED_ORIGINS` is a comma-separated browser/IdP origin allowlist (empty when unset); the ID origin itself is always trusted. Production requires non-empty origin-only `BETTER_AUTH_TRUSTED_ORIGINS` (no path or trailing slash) and `TRUSTED_PROXY_CIDRS`. `OPENAPI_ENABLED` defaults to false in production and true otherwise, with an explicit override. `DATABASE_POOL_MAX` defaults to 20 (1 in tests). Pool startup settings apply `DATABASE_STATEMENT_TIMEOUT_MS=10000`, `DATABASE_LOCK_TIMEOUT_MS=2000` and `DATABASE_IDLE_IN_TRANSACTION_TIMEOUT_MS=15000` to every connection. `GOOGLE_CLIENT_ID` with `GOOGLE_CLIENT_SECRET` and `MICROSOFT_CLIENT_ID` with `MICROSOFT_CLIENT_SECRET` configure the platform applications. Each pair must be supplied together or omitted together. Their secrets stay in the environment, never in platform SSO rows. Runtime settings declare their defaults in the same schema; see [`.env.example`](../../.env.example).
...
- All administrative mutations use an idempotency key with a permanent reservation; a retry returns the receipt. [Mutation inventory](../../reports/answerable-id-mutation-inventory.md).
